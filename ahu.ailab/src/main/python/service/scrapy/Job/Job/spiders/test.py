...
class UNDPjobSpider(scrapy.Spider):
    ...
    def __init__(self):
        ...
        self.noidziduan = ['Location :', 'Application Deadline :', 'Additional  Category :',
                           'Type of Contract :', 'Post Level :', 'Languages Required :',
        ...
        self.textnfo_noid = ['Background', 'Duties and Responsibilities', 'Competencies',
                ...
                if ziduanming[0] in self.noidziduan:
                    ...
                    if context:
                        if StrUtil.delWhite(ziduanming[0].strip(':')) == "LanguagesRequired":
                            item[StrUtil.delWhite(ziduanming[0].strip(':'))] = re.sub('\W', ' ',
                                                                                      StrUtil.delWhite(context[0]))
                        else:
                            item[StrUtil.delWhite(ziduanming[0].strip(':'))] = StrUtil.delMoreSpace(
                                StrUtil.delWhiteSpace(context[0]))
